...
// let networkName = "Wiki_Science_Filtered"
let networkName = "twitter"
	...
	xnet.loadXNETFile("./static/js/networks/" + networkName + ".xnet").then(async network => {
		...
		*/
		...
		for (let index = 0; index < nodeCount; index++) {
			nodes["" + index] = {
				ID: "" + index,
				// property: network.verticesProperties["Main Category"][index],
		...
		for (const [key, value] of Object.entries(network.edgesProperties)) {
			for (let index = 0; index < network.edges.length; index++) {
				edges[index][key.toLowerCase()] = value[index];
			}
		}
		...
		helios.onEdgeHoverEnd((edge, event) => {
			nodesHighlight([edge.source,edge.target],false);
			showTooltipForEdge(null);
		});

		helios.onEdgeClick((edge, event) => {
			if (edge) {
				console.log(`Edge clicked: ${edge.tweet}`);
			} else {
				console.log(`Clicked on background`);
			}
		});
